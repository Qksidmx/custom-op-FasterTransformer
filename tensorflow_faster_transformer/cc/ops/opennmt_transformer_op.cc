 ...
 */
#include "../kernels/opennmt_transformer_op.h"
#include "common_op.h"
...
REGISTER_OP("OpenNmtTransformer")
    .Input("from_tensor: T")
    .Input("to_tensor: T")
...
class OpenNmtTransformerOp : public CommonOp<T>
{
public:
  explicit OpenNmtTransformerOp(OpKernelConstruction *context) : CommonOp<T>(context)
  {
  ...
  void Compute(OpKernelContext *context) override
  {
    ...
    typedef OpenNmtEncoderTransformerTraits<traits_::OpType, cuda::OpenMultiHeadAttention> EncoderTraits_;
    OpenNmtEncoderTransformer<EncoderTraits_> *encoder_transformer_;
    fastertransformer::Allocator<AllocatorType::TF> allocator_(context);
    try
    {
      encoder_transformer_ = new OpenNmtEncoderTransformer<EncoderTraits_>(allocator_,
                                                                        batch_size_,
                                                                        from_seq_len_, 
                                                                        to_seq_len_, 
                                                                        head_num_, 
    ...
    this->get_tensor(context, 2, &param.self_layernorm.beta);
    this->get_tensor(context, 3, &param.self_layernorm.gamma);
    this->get_tensor(context, 4, &param.self_attention.query_weight.kernel);
    this->get_tensor(context, 5, &param.self_attention.key_weight.kernel);
    this->get_tensor(context, 6, &param.self_attention.value_weight.kernel);
    this->get_tensor(context, 7, &param.self_attention.query_weight.bias);
    this->get_tensor(context, 8, &param.self_attention.key_weight.bias);
    ...
    this->get_tensor(context, 13, &param.ffn_layernorm.beta);
    this->get_tensor(context, 14, &param.ffn_layernorm.gamma);
    ...
    OP_REQUIRES_OK(
        context,
        functor::OpenNmtTransformerOpFunctor<Device, T>::Compute(
            context,
            param,
...
#define REGISTER_GPU(T)                                                  \
  REGISTER_KERNEL_BUILDER(                                               \
      Name("OpenNmtTransformer").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
      OpenNmtTransformerOp<GPUDevice, T>)
REGISTER_GPU(float);
