 ...
 */
#include "../kernels/bert_transformer_op.h"
#include "common_op.h"
...
REGISTER_OP("SentenceBertTransformer")
    .Input("from_tensor: T")
    .Input("to_tensor: T")
    ...
    .SetShapeFn([](shape_inference::InferenceContext *c) {
      ...
      VLOG(2) << "[@SentenceBertTransformer::ShapeInference] batch_size: "
      ...
      c->set_output(1, c->MakeShape({output_dim1, head_num * size_per_head}));
      return Status::OK();
    });

template <typename Device, typename T>
class SentenceBertTransformerOp : public CommonOp<T>
{
public:
  explicit SentenceBertTransformerOp(OpKernelConstruction *context) : CommonOp<T>(context)
  {
  ...
  void Compute(OpKernelContext *context) override
  {
    ...
    VLOG(2) << "[@SentenceBertTransformer::Compute] getting batch size: "
            << batch_size_ << "\n";
...
#define REGISTER_GPU(T)                                                  \
  REGISTER_KERNEL_BUILDER(                                               \
      Name("SentenceBertTransformer").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
      SentenceBertTransformerOp<GPUDevice, T>)
REGISTER_GPU(float);
