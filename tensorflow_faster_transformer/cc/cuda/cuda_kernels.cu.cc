 ...
 */
#include "tensorflow_faster_transformer/cc/common.h"
#include "tensorflow_faster_transformer/cc/cuda/cuda_kernels.h"
#include <assert.h>
#include <cstdio>
#include <cstdlib>
#include <climits>
...
template <typename T>
__global__
void input_layernorm(T* out, const T* gamma, const T* beta, int m, int n)
{
    int tid = threadIdx.x;
...
void add_bias(T* out, const T* input, const T* bias, int m, int n)
{
    int tid = threadIdx.x;
    for(int i = tid; i < n; i += blockDim.x)
        out[blockIdx.x * n + i] = (float)(out[blockIdx.x * n + i] + input[blockIdx.x * n + i] + __ldg(&bias[i]));
}
...
//  dim3 grid(m / 64);
    ...
    assert(block.x <= 1024);
//  dim3 block(n);
    add_bias_relu_act<T><<<grid, block, 0, stream>>>(out, bias, m, n);
}
...
template<typename T>
void input_layernorm_kernelLauncher(T* out, const T* gamma, const T* beta, int m, int n, cudaStream_t stream)
{
    dim3 grid(m);
    dim3 block(n);
    assert(n <= 1024);
...
template<typename T>
void add_bias_kernelLauncher(T* out, const T* input, const T* bias,
                                             int m, int n, cudaStream_t stream)
{
    dim3 grid(m);
    dim3 block(n);
    assert(n <= 1024);
    add_bias<T><<<grid, block, 0, stream>>>(out, input, bias, m, n);
}
...
template void add_bias_kernelLauncher<half>(
    half* out, const half* input, const half* bias,
    int m, int n, cudaStream_t stream);
