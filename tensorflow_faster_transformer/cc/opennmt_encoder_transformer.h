...
class OpenNmtEncoderTransformerTraits;

template <template <OperationType> class MultiHeadAttention_>
class OpenNmtEncoderTransformerTraits<OperationType::FP32, MultiHeadAttention_>
...
template <template <OperationType> class MultiHeadAttention_>
class OpenNmtEncoderTransformerTraits<OperationType::FP16, MultiHeadAttention_>
...
class OpenNmtEncoderTransformer
{
  ...
  DataType_ *buf_;
  DataType_ *temp_attr_input_buf_;
  DataType_ *attr_out_buf_;
  DataType_ *attr_matmul_buf_;
  DataType_ *temp_attr_matmul_buf_;
...
public:
  OpenNmtEncoderTransformer(const IAllocator &allocator, int batch_size, int from_seq_len,
                                                                            ...
                                                                            to_seq_len_(to_seq_len), head_num_(head_num), size_per_head_(size_per_head)
  {
    ...
    try
    {
      buf_ = reinterpret_cast<DataType_ *>(allocator_.malloc(sizeof(DataType_) * buf_size * 8));
      if (buf_ == nullptr)
        throw std::runtime_error(std::string("Tensorflow Allocator failed to allocate internal buffer."));

      attr_out_buf_ = buf_;
      temp_attr_input_buf_ = attr_out_buf_ + buf_size;
      attr_matmul_buf_ = temp_attr_input_buf_ + buf_size;
      temp_attr_matmul_buf_ = attr_matmul_buf_ + buf_size;
      inter_matmul_buf_ = temp_attr_matmul_buf_ + buf_size;
  ...
  void forward()
  {
    ...
    try
    {
      ...
      int buf_size = m * n;
      //copy from_tensor -> add_bias
      check_cuda_error(cudaMemcpy(temp_attr_input_buf_, param_.from_tensor, sizeof(DataType_) * buf_size, cudaMemcpyDeviceToDevice));

      //layout_norm on input first
      input_layernorm_kernelLauncher<DataType_>(temp_attr_input_buf_,
                                                param_.self_layernorm.gamma,
                                                param_.self_layernorm.beta, m, n, param_.stream);

      cuda::MultiHeadInitParam<DataType_> multi_head_init_param;

      multi_head_init_param.from_tensor = temp_attr_input_buf_;
      multi_head_init_param.to_tensor = temp_attr_input_buf_;
      ...
      add_bias_kernelLauncher<DataType_>(attr_matmul_buf_,
      ...
      check_cuda_error(cudaGetLastError());
#endif
      //copy output -> add_bias
      check_cuda_error(cudaMemcpy(temp_attr_matmul_buf_, attr_matmul_buf_, sizeof(DataType_) * buf_size, cudaMemcpyDeviceToDevice));
      //ffn norm
      input_layernorm_kernelLauncher<DataType_>(attr_matmul_buf_,
      ...
      add_bias_relu_act_kernelLauncher<DataType_>(inter_matmul_buf_, param_.ffn.intermediate_weight.bias, m, n, param_.stream);
      ...
      add_bias_kernelLauncher<DataType_>(param_.transformer_out, temp_attr_matmul_buf_,
