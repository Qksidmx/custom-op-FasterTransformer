            ...
            if var in tf.trainable_variables():
                ...
                if var.name.find("Adam") != -1:
                    ...
                    if var.name.find("multi_head/conv1d/") != -1:
                        ...
                        ckpt[var.name.replace("multi_head/conv1d/", "multi_head/value/")] = V

                    elif var.name.find("multi_head/conv1d_1/") != -1:
                        name = var.name.replace("multi_head/conv1d_1/", "multi_head/dense/")
                        ckpt[name] = val
                    ...
                    # transformer/encoder/layer_x/ffn/conv1d_1/kernel:0 -> transformer/encoder/layer_x/output/dense/kernel:0
                        ...
                        name = var.name.replace("ffn/conv1d/", "ffn/dense/")
                        ckpt[name] = val
                        ...
                        name = var.name.replace("ffn/conv1d_1/", "ffn/dense_1/")
                        ckpt[name] = val

                    elif var.name.find("transformer/encoder/w_embs") != -1:
                        name = var.name
                        ckpt[name] = val
                    elif var.name.find("transformer/encoder/LayerNorm") != -1:
                        name = var.name
                        ckpt[name] = val
                        # Since a final layer normalization should be made on FT OpenNMT-encoder.
                        # The name is the the same as OpenNMT-encoder(without FT)
                        # We duplicate this name to make restore from OpenNMT checkpoints possible
                        name = var.name.replace("transformer/encoder/LayerNorm", "transformer/encoder/LayerNorm_1")
                        ckpt[name] = val
                    else:
                        name = var.name
                        ckpt[name] = val
